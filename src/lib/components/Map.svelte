...
<script lang="ts">
  ...
  function initializeMap(L: any) {
    ...
    // Define bounds based on the page type
    ...
    if (useBroadView) {
      // Configurasi untuk halaman tambah marker - view lebih luas
      southWest = L.latLng(-10.5, 104.0); // Diperluas ke selatan dan barat
      northEast = L.latLng(-3.0, 117.0); // Diperluas ke utara dan timur
      zoomLevel = isMobileSize ? 7 : (isTabletSize ? 7.5 : 8); // Menaikkan satu tingkat zoom dari sebelumnya
      minZoomLevel = zoomLevel; // Matikan zoom out dengan mengatur minZoom = zoomLevel saat ini
      boundsViscosity = 0.7; // Lebih fleksibel
    } else {
      // Configurasi untuk halaman utama - fokus pada Pulau Jawa
      southWest = L.latLng(-8.9, 105.0); // Batas selatan-barat Pulau Jawa
      northEast = L.latLng(-5.0, 116.0); // Batas utara-timur Pulau Jawa
      zoomLevel = isMobileSize ? 7 : (isTabletSize ? 7.5 : 8); // Sesuaikan zoom berdasarkan ukuran layar
      minZoomLevel = zoomLevel; // Matikan zoom out dengan mengatur minZoom = zoomLevel saat ini
      boundsViscosity = 0.9; // Lebih ketat tapi masih bisa digeser sedikit
    }
    ...
    map = L.map(mapElement, {
      center: [-6.9, 110.5], // Sesuaikan posisi center untuk lebih tepat di tengah
      zoom: zoomLevel,
    ...
    map.on('zoomend', function() {
      ...
      if (currentZoom < minZoomLevel) { 
        ...
        map.setZoom(minZoomLevel); // Kembalikan ke zoom minimal yang diinginkan
  ...
  function resetMapView() {
    if (map && mapInitialized) {
      ...
      // Tentukan level zoom berdasarkan jenis halaman dan ukuran layar
      let resetZoom;
      
      // Sesuaikan zoom berdasarkan ukuran layar - sama dengan inisialisasi awal
      if (window.innerWidth < 640) { // Mobile
        resetZoom = 7;
      } else if (window.innerWidth >= 640 && window.innerWidth < 1024) { // Tablet
        resetZoom = 7.5;
      } else { // Desktop
        resetZoom = 8;
      }
      
      // Kembalikan ke posisi default (Pulau Jawa)
      map.flyTo([-6.9, 110.5], resetZoom, {
