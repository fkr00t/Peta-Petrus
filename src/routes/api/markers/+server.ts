...
export const POST: RequestHandler = async (event) => {
  ...
  console.log('🎯 API POST /api/markers called');
  ...
  try {
    ...
    if (!body.title || typeof body.title !== 'string' || body.title.trim() === '') {
      console.log('❌ Title validation failed:', body.title);
    ...
    if (!body.latitude || !body.longitude || isNaN(parseFloat(body.latitude)) || isNaN(parseFloat(body.longitude))) {
      console.log('❌ Coordinates validation failed:', { lat: body.latitude, lng: body.longitude });
    ...
    if (body.categoryIds) {
      try {
        ...
        console.log('❌ Failed to parse categoryIds JSON:', body.categoryIds);
    ...
    if (categoryIds.length === 0) {
      console.log('❌ Categories validation failed:', { categoryId: body.categoryId, categoryIds: body.categoryIds });
      return json({ message: 'Minimal satu kategori harus dipilih' }, { status: 400 });
    }
    
    console.log('🏷️ Categories to assign:', categoryIds);
    ...
    if (existingCategories.length !== categoryIds.length) {
      ...
      console.log('❌ Some categories not found:', missingIds);
    ...
    console.log('✅ Marker created successfully with categories:', transformedResult);
