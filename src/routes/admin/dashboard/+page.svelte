          ...
          <div class="p-6">
            ...
            {:else}
              <div class="flex items-end space-x-4 h-32">
                {#each weeklyStats as stat, index}
                  <div class="flex-1 flex flex-col items-center">
                    <div 
                      ...
                      style="height: {weeklyStats.length > 0 ? (stat.markers / Math.max(...weeklyStats.map((s: any) => s.markers))) * 100 : 0}%"
                    ></div>
                    <div class="mt-2 text-xs font-medium text-gray-600 dark:text-gray-400">{stat.day}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-500">{stat.markers}</div>
                  </div>
                {/each}
              </div>
            {/if}
